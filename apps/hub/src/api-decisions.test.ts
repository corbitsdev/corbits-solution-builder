...
describe("AC3: the host holds no decision a body field could stale-check", () => {
  test("every command out of a stage gate is off the host; only effects remain", () => {
    ...
    for (const gate of gates) {
      if (gate === "stage.select_route" || gate === "stage.retry" || gate === "project.archive" || gate === "build.freeze") continue;
      expect(HOST_EFFECT_COMMANDS).not.toContain(gate);
    }
    expect(HOST_EFFECT_COMMANDS).toContain("build.freeze");
  });
  ...
  test("the route reads no expectedRevision or idempotencyKey for a decision", async () => {
    ...
    const commands = source.slice(source.indexOf('api.post("/projects/:projectId/commands/:command"'), source.indexOf('api.post("/projects/:projectId/build/start"'));
    expect(commands).not.toContain("expectedRevision");
    expect(commands).not.toContain("idempotencyKey");
