    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          "  exec)",
          "    mkdir -p src",
          '    printf \'{"name":"w","scripts":{"start":"bun run src/cli.ts"}}\' > package.json',
    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          "      mkdir -p src",
          '      printf \'{"name":"w","scripts":{"start":"bun run src/cli.ts","test":"exit 1"}}\' > package.json',
          "      echo 'console.log(\"built the thing\")' > src/cli.ts",
          "      echo 'first turn'",
          "    else",
          `      printf '%s' "$2" > ${JSON.stringify(promptLog)}`,
          '      printf \'{"name":"w","scripts":{"start":"bun run src/cli.ts","test":"exit 0"}}\' > package.json',
          "      echo 'second turn'",
          "    fi",
          "    ;;",
          "esac",
