...
function coverageSummary(targets: readonly TargetVerification[]): string {
  return targets
...
export function parseVerdict(text: string): { level: ConfidenceLevel; reasoning: string } | null {
  ...
  try {
    ...
    if (typeof parsed.level !== "string" || !LEVELS.includes(parsed.level as ConfidenceLevel)) return null;
