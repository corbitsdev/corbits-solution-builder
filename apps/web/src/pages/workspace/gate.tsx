...
export function StageGate({
    ...
    quorum === null
      ...
      : quorum.blocked > 0
        ...
        : quorum.proceeded < quorum.needed
          ...
          : null;
  // Advisory detail beneath the calm gate paragraph: where the stage-5
  // quorum stands and what the stage-1 evaluator said. Advisory only —
  // approval still follows quorumMet above.
  const advisoryParts: string[] = [];
  if (quorum !== null) {
    advisoryParts.push(
      `${quorum.proceeded} of ${quorum.named} stakeholders said proceed; needs ${quorum.needed}${quorum.blocked > 0 ? `; ${quorum.blocked} blocked` : ""}.`,
    );
  }
  if (evaluation !== null && evaluation.notes.length > 0) {
    advisoryParts.push(evaluation.notes.join(" "));
  }
  ...
  return (
    <div className="stage-gate" role="status">
      ...
      {advisory !== null ? <p className="stage-gate-note">{advisory}</p> : null}
      {soloApproval ? (
