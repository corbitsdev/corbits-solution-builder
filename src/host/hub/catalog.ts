 ...
 * `@intx/db/schema` — `orchestration/providers/registry.ts` reads and writes
 * the catalog only through the functions exported here.
 */
import { and, eq } from "drizzle-orm";
import { hub, hubIsMounted } from "./mount.js";
...
export async function listCatalogProviders(): Promise<CatalogProviderRow[]> {
  ...
  // (`src/host/hub/endpoint.ts`), which this reader does not yet. Reporting
