...
async function reachesHandshake(
  ...
  const child = Bun.spawn([...command, "--port", String(port)], {
    cwd: root,
    env: {
      ...process.env,
  ...
  return {
    ok: found,
    detail: found ? "" : (stderr.split("\n").find((line) => line.includes("error")) ?? "no handshake"),
    origin: found ? `http://127.0.0.1:${port}` : null,
    stop,
  };
}
...
{
  const result = await reachesHandshake(HOST_COMMAND, 8210);
...
if (await Bun.file(sidecar).exists()) {
  const result = await reachesHandshake([sidecar], 8211, { serveInterface: true });
