...
if (!Number.isInteger(stage) || stage < 1 || stage > 8) {
  console.error(`Usage: bun run seed --stage <1-8> (default 3) [--problem <path>] [--seed-artifacts]\nGot: ${arg("stage") ?? "(none)"}`);
  process.exit(1);
}
...
const { createProject, install, ensureLifecycleDeployment } = await import("./host-install.js");
